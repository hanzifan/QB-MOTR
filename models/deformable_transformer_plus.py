...
from .SVIT.test import Attention

class OnlyDeformableDecoder(nn.Module):
    ...
    def forward(self, idx, srcs, masks, query_embed=None, ref_pts=None, mem_bank=None, mem_bank_pad_mask=None, attn_mask=None):
        ...
        if self.two_stage:
            ...
            init_reference_out = reference_points
        # decoder
        hs, inter_references = self.decoder(idx, tgt, reference_points, memory,
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, idx, query_embed=None, ref_pts=None, mem_bank=None, mem_bank_pad_mask=None, attn_mask=None):
        assert self.two_stage or query_embed is not None
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # self.self_attn = Attention(embed_dim=d_model, num_heads=n_heads, attn_type='stream', num_frames=2)
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        ...
        # src2 = self.self_attn(self.with_pos_embed(src, pos), 0)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def _forward_self_cross(self, index, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index,
        ...
        # # 可视化 sampling locations: [bs, n, n_heads, n_levels, n_points, 2]
        # if sampling_locations.shape[1] > 100:
        #     sampling_locations = sampling_locations[:, 100:, :, :, :, :]
        ...
        #     # 分scale存储
        #     for i in range(sampling_locations.shape[2]):
        #         sampling_location_list.append(sampling_locations[:, :, i, :, :])
        #         attention_weight_list.append(attention_weights[:, :, i, :])
        #     # 合并所有head
        #     scales = [1280, 720]
        #     for i in range(sampling_locations.shape[2]):
        #         sampling_location_list[i] = rearrange(sampling_location_list[i], 'n n_heads n_points coord -> (n n_heads n_points) coord')
        #         attention_weight_list[i] = rearrange(attention_weight_list[i], 'n n_heads n_points -> (n n_heads n_points)')
        #         # 乘以尺度
        #         x = sampling_location_list[i][:, 0] * scales[0]
        #         y = sampling_location_list[i][:, 1] * scales[1]
        #         sampling_location_list[i] = torch.stack([x, y], dim=-1)
        ...
        #     thickness = 16 #  0 、4、8
        #     threshold = 0.1
        #     image_path = "/home/hzf/project/46/MOTRv2/exps/default/G1vjmJHr4_c004/frame_" + str(index) + ".jpg"
        #     save_path = "./visualization/" + str(index) + ".png"
        #     image = cv2.imread(image_path)
        #     for idx in range(len(sampling_location_list)):
        #         for idy in range(sampling_location_list[idx].shape[0]):
        #             if(attention_weight_list[idx][idy].item() > threshold):
        #                 weight = (attention_weight_list[idx][idy].item() - threshold) / (1.00 - threshold)
        #                 # weight = attention_weight_list[idx][idy].item()
        #                 point_color = (int(255 * (1.0 - weight)), 0, int(255 * weight)) #BGR
        #                 cv2.circle(image, (int(sampling_location_list[idx][idy][0].item()),int(sampling_location_list[idx][idy][1].item())), 
    ...
    def forward(self, idx, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None, mem_bank=None, mem_bank_pad_mask=None, attn_mask=None):
        if self.self_cross:
            return self._forward_self_cross(idx, tgt, query_pos, reference_points, src, src_spatial_shapes,
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, idx, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(idx, output, query_pos, reference_points_input, src, src_spatial_shapes,
...
class EncodingModel(nn.Module):
    def __init__(self, d_model, nhead, dropout=0.0):
        ...
        self.layers = _get_clones(self.multihead_attn, 4)

    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        return tensor if pos is None else tensor + pos

    def forward_once(self, layer, tgt, memory,
                memory_key_padding_mask: Optional[Tensor] = None,
                pos: Optional[Tensor] = None,
                query_pos: Optional[Tensor] = None):
        tgt2 = layer(query=self.with_pos_embed(tgt, query_pos),
    ...
    def forward(self, tgt, memory,
        ...
        for _, layer in enumerate(self.layers):
            output = self.forward_once(layer, output, memory, memory_key_padding_mask, pos, query_pos)
        return output
