...
class RuntimeTrackerBase(object):
    def __init__(self, score_thresh=0.45, filter_score_thresh=0.45, miss_tolerance=20):
...
class MOTR(nn.Module):
    ...
    def _forward_single_image(self, samples, track_instances: Instances, idx, is_last, gtboxes=None):
    ...
    def inference_single_image(self, img, ori_img_size, is_last, idx, track_instances=None, proposals=None):
        ...
        if track_instances is None:
            ...
            self.previous_instances = track_instances
        ...
        for frame_index, (frame, gt) in enumerate(zip(frames, data['gt_instances'])):
            ...
            if self.use_checkpoint and frame_index < len(frames) - 1:
                def fn(frame, gtboxes, *args):
                    ...
                    frame_res = self._forward_single_image(frame, tmp, 0, is_last, gtboxes)
                    return (
                ...
                frame = nested_tensor_from_tensor_list([frame])
                frame_res = self._forward_single_image(frame, track_instances, 0, is_last, gtboxes)
