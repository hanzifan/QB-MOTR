...
from .deformable_transformer_plus import build_deforamble_transformer, build_deforamble_decoder, pos2posemb, EncodingModel
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, decoder, num_classes, num_queries, memory_num_frames, num_feature_levels, criterion, track_embed,
        ...
        self.bbox_proj = nn.Linear(4, transformer.d_model, bias=False)
        self.independent_proj = nn.Linear(4, transformer.d_model, bias=False)
        ...
        self.boxquery_encoding = EncodingModel(d_model=hidden_dim, nhead=8)
    ...
    def _generate_empty_tracks(self, proposals=None):
        ...
        track_instances.box_query = torch.zeros((len(track_instances), d_model), dtype=torch.float, device=device)
    ...
    def clear_history(self):
        for track_query in self.history_track_query:
            del track_query
        self.history_track_query.clear()
    ...
    def Spatial_fusion(self, query, track_query, lvl):
        if track_query.shape[0] != 0:
            track_query = track_query.unsqueeze(0)
            rerank_memory = torch.cat([track_query, query], dim=1)
        else:
            rerank_memory = query

        fusion_query = self.spatial_fusion_module[lvl](tgt=query.permute(1,0,2),
                                          memory=rerank_memory.permute(1,0,2),
                                          memory_key_padding_mask=None,
                                          pos=None,
                                          query_pos=None
                                          )
        
        return fusion_query.permute(1,0,2)

    def _forward_single_image(self, samples, track_instances: Instances, is_last, idx, gtboxes=None):
        # features, pos = self.backbone(samples, is_last)
        ...
        # MEG
        box_query = self.bbox_proj(track_instances.pred_boxes.detach())
        # box_query = self.boxquery_encoding(tgt=box_query.unsqueeze(0).permute(1,0,2), 
        #                                    memory=torch.cat([box_query, track_instances.box_query.detach()], dim=0).unsqueeze(0).permute(1,0,2)).permute(1,0,2).squeeze(0)
        # box_query = self.boxquery_encoding(tgt=box_query.unsqueeze(0).permute(1,0,2), 
                                        #    memory=box_query.unsqueeze(0).permute(1,0,2)).permute(1,0,2).squeeze(0)
        # independent_box_query = self.independent_proj(track_instances.pred_boxes.detach())
        box_hs, box_init_reference, box_inter_references, box_enc_outputs_class, box_enc_outputs_coord_unact = \
            self.decoder(idx, srcs, masks, box_query, ref_pts=track_instances.ref_pts.detach(),
                        mem_bank=track_instances.mem_bank.detach(), mem_bank_pad_mask=track_instances.mem_padding_mask.detach(), attn_mask=attn_mask)
        # update box_query
        # track_instances.box_query = box_query
        # res architecture
        box_query += box_hs[-1].squeeze(0).detach()
        # adding query bootstarpping embedding
        query_embed = query_embed + box_query

        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = \
            self.transformer(srcs, masks, pos, idx, query_embed, ref_pts=ref_pts,
    ...
    @torch.no_grad()
    def inference_single_image(self, img, ori_img_size, is_last, idx, track_instances=None, proposals=None):
        ...
        if track_instances is None:
            ...
            track_instances = Instances.cat([
                ...
                track_instances])
        res = self._forward_single_image(img, track_instances, idx, is_last)
        ...
        # for frame_index, (frame, gt) in enumerate(zip(frames, data['gt_instances'])):
            ...
            if self.use_checkpoint and frame_index < len(frames) - 1:
                def fn(frame, gtboxes, *args):
                    ...
                    frame_res = self._forward_single_image(frame, tmp, is_last, gtboxes)
                    return (
                ...
                frame = nested_tensor_from_tensor_list([frame])
                frame_res = self._forward_single_image(frame, track_instances, is_last, gtboxes)
...
def build(args):
    ...
    model = MOTR(
        ...
        memory_num_frames = args.memory_num_frames,
