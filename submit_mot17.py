...
def plot_one_box(x, img, color=None, label=None, score=None, line_thickness=None):
    ...
    if label:
        ...
        if score is not None:
            cv2.putText(img.numpy(), score, (c1[0], c1[1] + 30), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
    return img
...
class Detector(object):
    ...
    def visualize_img_with_bbox(img_path, img, dt_instances: Instances, ref_pts=None, gt_boxes=None):
        ...
        cv2.imwrite(img_path, img_show.numpy())

    def detect(self, prob_threshold=0.5, area_threshold=100):
        ...
        for i, (cur_img, ori_img) in enumerate(tqdm(loader)):
            ...
            res = self.detr.inference_single_image(cur_img.cuda().float(), (seq_h, seq_w), is_last, i, track_instances)
...
if __name__ == '__main__':
    ...
    seq_nums = os.listdir('/home/hzf/data/MOT17/images/train/')
    accs = []
    seqs = []
    for seq_num in seq_nums:
