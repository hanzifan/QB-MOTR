...
def convert(img_dir, split, label_dir, save_label_dir, filter_crowd=False, filter_ignore=False):
    ...
    for vid in tqdm(vids):
        ...
        txt_label_dir = os.path.join(save_label_dir, vid)
        ...
        for anno in annos:
            ...
            with open(os.path.join(txt_label_dir, name.replace('jpg', 'txt')[-9:]), 'w') as f:
                for label in labels:
                    obj_id = label['id']
                    category = label['category']
    ...
    for vid in tqdm(all_vids):
        ...
        for fid in fids:
            if os.path.exists(os.path.join(label_dir, vid, fid.replace('jpg', 'txt'))):
                all_frames.append(f'JPEGImages/{vid}/{fid}')
    with open(txt_path, 'w') as f:
        for frame in all_frames:
            f.write(frame+'\n')

# img_dir = '/home/hzf/data/Refer-Youtube_VIS/train/JPEGImages/'
# label_dir = '/home/hzf/data/Refer-Youtube_VIS/train/labels/'
# save_label_dir = '/home/hzf/data/Refer-Youtube_VIS/train/txt_labels/'
# split = 'train'
...
save_label_dir = '/home/hzf/data/Refer-Youtube_VIS/train/txt_labels/'
split = 'train'
generate_txt(img_dir,save_label_dir,txt_path='youtube-vis.train',split='train')
